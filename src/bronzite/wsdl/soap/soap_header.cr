  ...
  module Wsdl
    module Soap
      class SoapHeader
        ...
        @namespace : String?
        @header_faults : Array(SoapHeaderFault)
        ...
        def self.parse(node : XML::Node, ctx : Bronzite::Document)
          ...
          sh_header_faults = faults.reduce([] of SoapHeaderFault) do |memo, e|
            memo.push(SoapHeaderFault.parse(e, ctx))
            memo
          end

          SoapHeader.new(sh_use, sh_message, sh_part, sh_encoding_style, sh_namespace, sh_header_faults)
        end
